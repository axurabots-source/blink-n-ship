...
function TeamManagementTab() {
  ...
  const handleInvite = async (formData: any) => {
    ...
    try {
      ...
      setShowInvite(false);
      setSuccess(`Invited ${formData.name}`);
  ...
  const handleSavePermissions = async (memberId: string, permissions: any[]) => {
    setSaving(true); clearMessages();
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      setMembers(prev => prev.map(m => m.id === memberId ? { ...m, permissions: data.permissions } : m));
  ...
  if (loading) {
    return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, gap: 12 }}>
      <div style={{ width: 24, height: 24, borderRadius: '50%', border: `2px solid ${T.accentLight}`, borderTopColor: T.accent, animation: 'bns-spin 0.6s linear infinite' }} />
      <span style={{ fontSize: '0.82rem', color: T.muted }}>Loading team...</span>
    </div>;
  }
  ...
  const statusBadge = (status: string) => {
    const styles: Record<string, any> = {
      ...
      invited: { bg: '#fefce8', color: '#ca8a04' },
      suspended: { bg: '#fef2f2', color: '#dc2626' },
    };
    const s = styles[status] || styles.invited;
    return <span style={{ fontSize: '0.65rem', fontWeight: 700, padding: '2px 10px', borderRadius: 20, background: s.bg, color: s.color, textTransform: 'uppercase', letterSpacing: '0.03em' }}>{status}</span>;
  };
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h2 style={{ fontSize: '1rem', fontWeight: 700, color: T.fg, margin: '0 0 4px' }}>Team Management</h2>
          <p style={{ fontSize: '0.8rem', color: T.muted, margin: 0 }}>Manage your team members and their permissions.</p>
        </div>
        <button onClick={() => { setShowInvite(true); setEditingMember(null); setEditingPermissions(null); clearMessages(); }} style={{
          display: 'flex', alignItems: 'center', gap: 8, background: T.accent, color: '#fff', border: 'none',
          borderRadius: 10, padding: '10px 20px', fontSize: '0.82rem', fontWeight: 600, cursor: 'pointer',
        }}>
      ...
      {showInvite && <InviteForm onInvite={handleInvite} onCancel={() => setShowInvite(false)} saving={saving} />}
      {editingMember && <EditMemberForm member={editingMember} onSave={handleUpdateMember} onCancel={() => setEditingMember(null)} saving={saving} />}
      {editingPermissions && <PermissionsForm member={editingPermissions} onSave={handleSavePermissions} onCancel={() => setEditingPermissions(null)} saving={saving} />}

      {members.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '48px 20px', background: T.inputBg, borderRadius: 12, border: `1px dashed ${T.border}` }}>
          <Users size={36} color={T.muted} style={{ margin: '0 auto 12px', opacity: 0.4 }} />
          <p style={{ fontSize: '0.9rem', fontWeight: 600, color: T.fg, margin: '0 0 4px' }}>No team members yet</p>
          <p style={{ fontSize: '0.8rem', color: T.muted, margin: 0 }}>Invite your first team member to get started.</p>
        </div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {members.map(member => (
            <div key={member.id} style={{
              ...
              padding: '14px 16px', background: T.bg, borderRadius: 10, border: `1px solid ${T.border}`,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flex: 1, minWidth: 200 }}>
                ...
                <div>
                  <p style={{ fontSize: '0.85rem', fontWeight: 600, color: T.fg, margin: 0 }}>{member.name}</p>
                  <p style={{ fontSize: '0.75rem', color: T.muted, margin: '2px 0 0' }}>{member.email} · {member.role}</p>
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                {statusBadge(member.status)}
                <button onClick={() => { setEditingPermissions(member); setEditingMember(null); setShowInvite(false); clearMessages(); }} style={{ border: 'none', background: 'none', color: T.accent, cursor: 'pointer', padding: 4 }} title="Permissions">
                  <Shield size={15} />
                </button>
                <button onClick={() => { setEditingMember(member); setShowInvite(false); setEditingPermissions(null); clearMessages(); }} style={{ border: 'none', background: 'none', color: T.muted, cursor: 'pointer', padding: 4 }} title="Edit">
                  <Pencil size={14} />
                </button>
                <button onClick={() => handleDeleteMember(member.id)} disabled={saving} style={{ border: 'none', background: 'none', color: '#dc2626', cursor: saving ? 'not-allowed' : 'pointer', padding: 4, opacity: saving ? 0.5 : 1 }} title="Delete">
                  <Trash2 size={14} />
                </button>
              </div>
            </div>
